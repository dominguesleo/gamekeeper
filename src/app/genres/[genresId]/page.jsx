...
async function getGenreGames(page, genresId, retries = 5) {
    try {
        const response = await fetch(`https://api.rawg.io/api/games?page=${page}&genres=${genresId}&key=a359d27af5fc427c87fb3af6dd0b91b4`);
        if (!response.ok) {
...
async function getGenreName(genresId, retries = 5) {
    try {
        const response = await fetch(`https://api.rawg.io/api/genres/${genresId}?key=a359d27af5fc427c87fb3af6dd0b91b4`);
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
