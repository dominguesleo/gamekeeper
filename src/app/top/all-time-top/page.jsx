...
async function getAllTimeTop(page, retries = 10) {
    try {
        const response = await fetch(`https://api.rawg.io/api/games?page=${page}&key=${process.env.NEXT_PUBLIC_RAWG_TOKEN}&ordering=-rating`);
        if (!response.ok) {
